import { mysqlTable, mysqlSchema, AnyMySqlColumn, char, varchar, json, index, foreignKey, primaryKey, timestamp, int, tinyint, bigint, unique, longtext } from "drizzle-orm/mysql-core"
...
export const course = mysqlTable("course", {
	id: char("id", { length: 36 }).notNull(),
	courseName: varchar("courseName", { length: 255 }).notNull(),
	subject: char("subject", { length: 36 }).references(() => subject.id, { onUpdate: "cascade" } ),
...
export const notes = mysqlTable("notes", {
	id: char("id", { length: 36 }).notNull(),
	topic: char("topic", { length: 36 }).notNull(),
	subjectRef: char("subjectRef", { length: 36 }).notNull(),
...
(table) => {
	return {
		...
		subjectRef: index("subjectRef").on(table.subjectRef),
...
export const users = mysqlTable("users", {
	...
	userPassword: varchar("userPassword", { length: 255 }),
	refreshToken: varchar("refreshToken", { length: 255 }),
	spaceRepetition: longtext("spaceRepetition"),
